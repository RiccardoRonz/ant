...
class Ant:
    ...
    def _get_markov_matrices(self) -> None:
        ...
        transition_matrix = pd.DataFrame(index=points_i, columns=points_i).fillna(0)
        for point in self.points:
            x,y = point
            admissible_adiacents = list(self._get_adiacent(x,y).intersection(points_s))
        ...
        n = pd.DataFrame(np.linalg.inv((identity-transition_matrix.values)), index=points_i, columns=points_i)
        solutions = n.dot(np.ones((len(points_i),1))).rename(columns={0:'avg_steps'})
    ...
    def get_markov_solution(self) -> None:
        ...
        print(f'Bill has to make {avg_steps} steps on average to reach the food. Poor Bill!')
    ...
    def do_run(self) -> int:
        ...
        while self.eval_bound(x,y):
            rnd = random.uniform(0,1)
            if rnd < 0.5: x += self.take_step()
            else: y += self.take_step()
            steps += 1
        return steps
    ...
    def do_monte_carlo_avg_steps(self, n_samples = 100, n_runs = 1000) -> pd.DataFrame():
        ...
        if self.workers is not None:
            with multiprocessing.Pool(processes=self.workers) as pool:
                avg_steps = pool.map(self.calculate_avg_steps, [n_runs for _ in range(n_samples)])
        else:
            avg_steps = [self.calculate_avg_steps(n_runs=n_runs) for _ in range(n_samples)]
        data = pd.DataFrame(data=avg_steps, columns=['average_steps'])
        print(f'Bill has to make {data["average_steps"].mean()} steps on average to reach the food. Poor Bill!')
        return data
    
    @lru_cache(maxsize = 100000000)
    def get_probability_recusive(self,t,p):
        if (t == 0) and (p == (0,0)):
            return 1
        if t == 0:
            return 0
        ...
        return sum([0.25*self.get_probability_recusive(t-1,adiacent) for adiacent in admissible_adiacents])
    
    def get_first_non_transition_points(self):
        all_points = set()
        for point in self.points:
            all_points = all_points.union(self._get_adiacent(point[0], point[1]))
